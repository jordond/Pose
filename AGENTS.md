...
|----------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `annotations/`       | `@Pose`, `@PoseSample`, `@PoseIgnore`, plus `PoseConfig` / `@PoseSetup`. **Kotlin Multiplatform** (jvm, iosArm64, iosSimulatorArm64, macosArm64, js, wasmJs) so CMP `commonMain` can depend on it; sources live in `src/commonMain/kotlin`. Applies the Compose compiler plugin with `compose-runtime` as **compileOnly**, so the published POM still has zero Compose deps. Android consumers resolve the `jvm` variant. Published to Central.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| `sample-app/`        | End-to-end smoke test. Every capability of Pose is exercised in one file each ([LoginContent](sample-app/src/commonMain/kotlin/io/github/akshaychordiya/pose/sample/LoginContent.kt), [HomeContent](sample-app/src/commonMain/kotlin/io/github/akshaychordiya/pose/sample/HomeContent.kt), [ProductCard](sample-app/src/commonMain/kotlin/io/github/akshaychordiya/pose/sample/ProductCard.kt), [ArticleCard](sample-app/src/commonMain/kotlin/io/github/akshaychordiya/pose/sample/ArticleCard.kt), [WellKnownTypesCard](sample-app/src/commonMain/kotlin/io/github/akshaychordiya/pose/sample/WellKnownTypesCard.kt), [SelfThemedBanner](sample-app/src/commonMain/kotlin/io/github/akshaychordiya/pose/sample/SelfThemedBanner.kt)). If a change plausibly affects generation, verify it here too. |
| `sample-android/`    | Android entry point for the sample - `MainActivity`, manifest, APK. Exists as its own module only because AGP 9 made `com.android.application` incompatible with the Kotlin Multiplatform plugin in the same module. No Pose-specific code lives here.                                                                                                                                                                                                                                                                                                          |
...
| `docs/design/`       | Design notes for changes big enough to need one written down before coding (e.g. type-safe config).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| Find + validate the module's `@PoseSetup` config object                                | `PoseSetupResolver`       |
...
# Verify previews generate, then that they compile on every target
./gradlew :sample-app:kspCommonMainKotlinMetadata
# then inspect: sample-app/build/generated/ksp/metadata/commonMain/kotlin/io/github/akshaychordiya/pose/sample/
./gradlew :sample-app:build

# Runnable Android app (MainActivity + APK)
...
Follow the existing pattern in `sample-app/src/commonMain/kotlin/…/sample/` - one composable per file with a `Showcase - <what this demonstrates>` comment at the top. Keep each file small and focused on one axis.
...
- ❌ Add non-`compileOnly` dependencies to the `annotations` module. Consumers must not pull Compose tooling transitively just by depending on the markers. Verify with `./gradlew :annotations:generatePomFileForMavenPublication` — `annotations/build/publications/maven/pom-default.xml` should list only `kotlin-stdlib`.
- ❌ Move `PoseConfig`'s scalar settings from `@PoseSetup` onto the interface as properties. KSP can read annotation arguments but **not** property initializers, so `override val maxDepth = 12` would be invisible to the processor.
