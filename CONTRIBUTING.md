...
|---------------------------------------------|---------------------------------------------------------------------------------|--------------------------------|
| `io.github.akshaychordiya.pose:annotations` | `@Pose`, `@PoseProvider`, `@PoseIgnore` (pure Kotlin JVM JAR, no Compose deps)  | `implementation`               |
...
The annotations JAR is intentionally minimal — three annotations, no runtime dependencies. Consumers should not accidentally pull in Compose tooling just by depending on the marker. **Don't add dependencies to it.**
...
- Cover it in `OptionsTest.kt`, plus a behavioural test if it changes emitted output
